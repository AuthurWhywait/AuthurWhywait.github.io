...
In FCOS, we directly <font color=blue>limit the range of bounding box regression for each level</font>.

1. Regression targets $l^*,t^*,r^*$ and $b^*$ for each location on all feature levels.
2. If a location satisfies $\max(l^*,t^*,r^*,b^*)>m_i$ or $\max(l^*,t^*,r^*,b^*)<m_{i-1}$, it is set as a negative sample. (not require to regress a bounding box) ($m_i$ is the maximum distance that feature level $i$ need to regress) In this work $m2,...,m7=0,64,128,256,512,\infty$)
